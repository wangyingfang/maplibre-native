...
void TilePlacement::placeSymbolBucket(const BucketPlacementData& params, std::set<uint32_t>& seen) {
    ...
    auto symbolIntersectsTileEdges = [&collisionBoxIntersectsTileEdges,
        ...
        if (symbol.getTextVariableAnchorOffset() && !symbol.getTextVariableAnchorOffset()->empty()) {
            variableAnchor = symbol.getTextVariableAnchorOffset()->begin()->anchorType;
        }
        ...
        if (!symbol.getTextCollisionFeature().boxes.empty()) {
            ...
            if (variableAnchor) {
                ...
                auto variableTextOffset = symbol.getTextVariableAnchorOffset()->getOffsetByAnchor(*variableAnchor);
